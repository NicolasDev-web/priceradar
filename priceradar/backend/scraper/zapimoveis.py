...
def _parse_json_ld(
    ...
    # Mesma plataforma do VivaReal: o payload RSC do mesmo html traz coordenada
    # e bairro estruturado, sem requisição nova.
        ...
        for entry in items:
            ...
            try:
                ...
                registro = {
                    ...
                    'fotos': extrair_fotos(item.get('image') or item.get('photo'), ZAP_BASE_URL),
                }
    ...
    logger.info(f"Zap: {com_coord}/{len(resultados)} anúncios com coordenada")
    return resultados
...
async def scrape_zapimoveis(
    ...
    resultados = await paginar(
        lambda p: _fetch_pagina_zap(cidade_nome, estado_lower, preco_min, preco_max, quartos, bairro, p, cidade_normalizada),
        MAX_PAGINAS,
        "Zap",
    )
    return resultados
